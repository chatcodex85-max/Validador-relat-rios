...
def first_matching_column(headers: dict[str, int], candidates: Iterable[str]) -> str | None:
...
def detect_report_type(headers: dict[str, int]) -> str:
    ...
    if first_matching_column(headers, ["NumeroLogradouro", "Numero Logradouro", "N Logradouro"]):
...
def read_headers(sheet) -> dict[str, int]:
    ...
    for index, cell in enumerate(next(sheet.iter_rows(min_row=1, max_row=1, values_only=True)), start=1):
        if cell is None:
            continue
...
def validate_workbook(path: Path) -> list[ValidationErrorRow]:
    ...
    for sheet in workbook.worksheets:
        ...
        numero_col = first_matching_column(headers, ["NumeroLogradouro", "Numero Logradouro", "N Logradouro"])
        ...
        capacidade_col = first_matching_column(
            headers, ["CapacidadeIndicada", "Capacidade Atual", "CapacidadeAtual", "CapacidadeIndicacao"]
        )
        taxa_col = first_matching_column(headers, ["TaxaIndicacao", "Taxa Indicacao", "Porcentagem"])
        ...
        nome_col = first_name_column(
            headers, ["DescricaoLocalProva", "Descricao Local Prova", "LocalProva", "DescricaoCidade"]
        )
        uf_col = first_matching_column(headers, ["UF", "SiglaUF", "Sigla UF"])
        cidade_col = first_matching_column(headers, ["Cidade", "Municipio", "DescricaoCidade"])
                ...
                if tag:
                    errors.append(
                        build_result(
                            ...
                            exibido=display(taxa_value) or format_percentage(percentage),
...
def run_cli(args: argparse.Namespace) -> int:
    ...
    print("ID;UF;Cidade;Nome;Exibido;Erro;Tag")
    for error in errors:
        print(
            ";".join(
                [
...
def run_gui() -> None:
    ...
    def columns_for_errors(errors: list[ValidationErrorRow]) -> list[str]:
        if errors and all(error.tipo_relatorio == "Meta capacidade" for error in errors):
            return ["id", "uf", "cidade", "nome", "meta", "capacidade_atual", "porcentagem", "tag"]

        columns = ["id", "uf", "cidade", "nome"]
        optional_columns = ["meta", "capacidade_atual", "porcentagem", "exibido", "erro", "tag"]
    ...
    def apply_theme() -> None:
        ...
        style.configure("TCombobox", fieldbackground=field, background=panel, foreground=fg)
        style.configure(
            "Treeview",
            background=panel,
        ...
        style.configure("Treeview.Heading", background=field, foreground=fg, font=("Segoe UI", 9, "bold"))
    ...
    uf_combo = ttk.Combobox(filter_frame, textvariable=uf_var, state="readonly", width=10, values=["Todas"])
    uf_combo.grid(row=1, column=1, padx=(0, 10), sticky=tk.EW)

    ttk.Label(filter_frame, text="Cidade").grid(row=0, column=2, sticky=tk.W)
    cidade_combo = ttk.Combobox(filter_frame, textvariable=cidade_var, state="readonly", width=22, values=["Todas"])
    cidade_combo.grid(row=1, column=2, padx=(0, 10), sticky=tk.EW)

    ttk.Label(filter_frame, text="Erro").grid(row=0, column=3, sticky=tk.W)
    erro_combo = ttk.Combobox(filter_frame, textvariable=erro_var, state="readonly", width=18, values=["Todos"])
    erro_combo.grid(row=1, column=3, padx=(0, 10), sticky=tk.EW)

    ttk.Label(filter_frame, text="Tag").grid(row=0, column=4, sticky=tk.W)
    tag_combo = ttk.Combobox(filter_frame, textvariable=tag_var, state="readonly", width=22, values=["Todas"])
    ...
    def render_table() -> None:
        ...
        for error in errors:
            table.insert(
                "",
                tk.END,
                values=tuple(value_for_column(error, column) for column in current_columns),
            )

        status.config(text=f"{len(errors)} item(ns) exibido(s) de {len(all_errors)} encontrado(s). Clique no ID para copiar.")

    def update_filter_options(errors: list[ValidationErrorRow]) -> None:
        uf_combo["values"] = ["Todas"] + sorted({error.uf for error in errors if error.uf})
        cidade_combo["values"] = ["Todas"] + sorted({error.cidade for error in errors if error.cidade})
        erro_combo["values"] = ["Todos"] + sorted({error.erro for error in errors if error.erro})
        tag_combo["values"] = ["Todas"] + sorted({error.tag for error in errors if error.tag})
        search_var.set("")
        uf_var.set("Todas")
    ...
    ttk.Button(top_frame, text="Selecionar .xlsx", command=choose_files).pack(side=tk.RIGHT, padx=(8, 0))
    ttk.Button(top_frame, text="Validar", command=validate_selected).pack(side=tk.RIGHT, padx=(8, 0))
    theme_button = ttk.Button(top_frame, text="Tema escuro", command=toggle_theme)
    theme_button.pack(side=tk.RIGHT, padx=(8, 0))
    ttk.Button(filter_frame, text="Filtrar", command=render_table).grid(row=1, column=5, padx=(0, 8))
    ttk.Button(filter_frame, text="Limpar filtros", command=clear_filters).grid(row=1, column=6)
